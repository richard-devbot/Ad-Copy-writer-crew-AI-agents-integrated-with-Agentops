...
class MarketingAnalysisTasks:
	def product_analysis(self, agent, product_website, product_details):
		return Task(description=dedent(f"""\
			...
			"""),
			agent=agent,
			expected_output="A comprehensive analysis of the product's key selling points, market appeal, and suggestions for enhancement or positioning."
		)
	...
	def competitor_analysis(self, agent, product_website, product_details):
		return Task(description=dedent(f"""\
			...
			"""),
			agent=agent,
			expected_output="A detailed comparison of the product with its top 3 competitors, including their strategies, market positioning, and customer perception."
		)
	...
	def campaign_development(self, agent, product_website, product_details):
		return Task(description=dedent(f"""\
			...
			"""),
			agent=agent,
			expected_output="A targeted marketing campaign strategy and creative content ideas that resonate with the product's target audience."
		)
	...
	def instagram_ad_copy(self, agent):
		return Task(description=dedent("""\
			...
			"""),
			agent=agent,
			expected_output="Three options for an engaging Instagram post copy that resonates with the target audience and highlights the product's unique selling points."
		)
	...
	def take_photograph_task(self, agent, copy, product_website, product_details):
		return Task(description=dedent(f"""\
			...
			"""),
			agent=agent,
			expected_output="Three options for an Instagram post photograph that captures the audience's attention and aligns with the product's marketing strategy."
		)
	...
	def review_photo(self, agent, product_website, product_details):
		return Task(description=dedent(f"""\
			...
			"""),
			agent=agent,
			expected_output="A review of the photo, with a paragraph describing the photo and a score from 1 to 10."
		)